...
class WPCacheImpl implements CacheInterface  {
	...
	function __construct( $cache_id = null ) {
		if ( ! $cache_id ) {
			$this->cache_id = 'aretex_default_cache_id';
		} else {
	...
	public function set( $key, $value, $expires = 0 ) {
		if ( ! set_transient( $key, $value, $expires )  ) {
			...
			$cache_keys = get_aretex_session( $this->cache_id , array() );
			$cache_keys[] = $key;
			update_aretex_session( $this->cache_id , $cache_keys );
		}
	}
	...
	public function clear_cache( $id = false ) {
		...
		$cache_keys = get_aretex_session( $id , array() );
